...
In order to push the project's AI-assisted philosophy as much as possible, the specifics were not written by hand. Rather, I sent the [design document](./Retriva-SDD_implementation_1.pdf) to MS Copilot (which I created by brainstorming with Copilot itself) so that it could write the spec files in accordance with Antigravity's conventions.
